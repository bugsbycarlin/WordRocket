console.log("listening for device ready");
document.addEventListener("deviceready", function() { console.log("it is read"); onDeviceReady();}, false);
...
class Game {
  ...
  initialize() {
    ...
    this.scenes["title"].scale.set(0.5, 0.5);
    this.scenes["solo"].scale.set(0.5, 0.5);
  ...
  makeQwertyPalette(parent, size, x, y, add_special_keys, action) {
    var self = this;
    var palette = new PIXI.Container();
    parent.addChild(palette);
    palette.position.set(x, y);
...
function onDeviceReady() {
  if(device.platform == "iOS") {screen.orientation.lock("portrait-primary");}
  game = new Game();
}
